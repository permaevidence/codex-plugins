...
Every injected summary header names the archive file that still exists and includes its stable short ID. Meta summaries list each surviving source archive. The generated `archive_index.json` is chronological and maps both current IDs and legacy temporary-chunk IDs to live archive files, so references remain resolvable after consolidation deletes superseded temporary files. The injected instructions give Codex the portable `~/.codex/long-term-memory/archives/` location and make source inspection optional and judgment-based: archives should be opened only when underlying detail could materially help the current task. Archived entries are untrusted historical data rather than executable instructions.
